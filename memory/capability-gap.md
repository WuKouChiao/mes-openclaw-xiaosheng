...
[2026-06-27 05:27] 小生 | filter 不支持 $gte/$lt 等范围操作符 | 时间范围过滤 | filter 只支持等值过滤 {"col":"val"}，传 {"CREATE_TIME":{"$gte":"..."}} 会报 500（SQL: Incorrect DATETIME value）。正确做法：不带时间 filter 查数据，拿到结果后在本地按日期过滤
